...
<script type="text/javascript">
			...
			url:"${pageContext.request.contextPath}/pro/withAnalyseJsonList",
             success:function (msg) {
				 $(msg).each(function (index,item) {
					 $("#prjects").append("<option value='"+item.pid+"'>"+item.pname+"</option>")
                 })
             }
		})
    })
	function parseAnalysis(pid) {
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/anal/info"+pid,
            success:function (msg) {
        ...
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/module/info"+list,
            success:function (msg) {
                $(msg).each(function (index,item) {
                    $("#prjects").append("<option value='"+item.pid+"'>"+item.pname+"</option>")
                })
...
<tr >
	...
	<td  align='left' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
		<select id="prjects" onchange="parseAnalysis(this.value)">
			<option>请选择</option>
		</select>-
		<select id="analysis">
			<option>请选择</option>
		...
		<select id="moudle" onchange="parseFunction(this.value)">
			<option>请选择</option>
